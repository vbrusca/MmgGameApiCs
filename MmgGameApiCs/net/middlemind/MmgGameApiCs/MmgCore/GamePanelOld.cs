...
namespace net.middlemind.MmgGameApiCs.MmgCore
{
    ...
    public class GamePanelOld : Game, GenericEventHandler, GamePadSimple
    {
        /// <summary>
        ...
        public GamePanelOld(MainFrame Mf, int WinWidth, int WinHeight, int X, int Y, int GameWidth, int GameHeight)
        {
            gdm = new GraphicsDeviceManager(this);
            mf = Mf;
            winWidth = WinWidth;
            winHeight = WinHeight;
            GamePanelOld.GAME_WIDTH = GameWidth;
            GamePanelOld.GAME_HEIGHT = GameHeight;
        ...
        protected override void LoadContent() {
            ...
            screenData = new MmgScreenData(winWidth, winHeight, GamePanelOld.GAME_WIDTH, GamePanelOld.GAME_HEIGHT);
            MmgHelper.wr("");
        ...
        public virtual void RenderGame()
        {
            ...
            else
            {
                ...
                */
                ...
                p.GetSpriteBatch().Begin();
                currentScreen.MmgDraw(p);
                p.GetSpriteBatch().End();
            ...
            if (scale != 1.0)
            {
                p.GetGraphics().Draw(background, new Rectangle(sMyX, sMyY, sWinWidth, sWinHeight), new Rectangle(0, 0, winWidth, winHeight), Color.White);

            }
            else
            {
                p.GetGraphics().Draw(background, new Vector2(myX, myY), Color.White);
            }
            p.GetSpriteBatch().End();
